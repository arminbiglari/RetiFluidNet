...
class Results:
    ...
    def __init__(self, args):
        self.layers_names = args.classes_names
        self.args = args
    ...
    def results_per_layer(self, predictions, labels):
        ...
        pred_data = np.zeros(shape = (num_samples, self.args.image_size, self.args.image_size, self.args.num_classes))
        for i in range(num_samples):
            sample = predictions[i]
            sample = np.expand_dims(sample, axis = 0)
            for x in range(self.args.num_classes):
                output_bicon = self.bv_test(sample[:, :, :, x * 8:(x + 1) * 8])  
                pred_data[i, :, :, x] = np.expand_dims(output_bicon, axis = 0)
            
        predictions = pred_data
        predictions = np.squeeze(predictions)        
        predictions = np.argmax(predictions, axis = -1)
        predictions = np.array(tf.keras.utils.to_categorical(predictions , 2))
        ...
        for image, label in labels:    
            ...
            labels_.append(tf.keras.backend.one_hot(tf.cast(label, 'int32'),num_classes = self.args.num_classes)) 
        print("\n")    
        ...
        for i in range(number_of_samples):
            ...
            for j in range(len(self.layers_names)):
                acc_dict[self.layers_names[j]]       += acc_value[i][self.layers_names[j]]            
                f1_score_dict[self.layers_names[j]]  += f1_score_value[i][self.layers_names[j]]
                precision_dict[self.layers_names[j]] += precision_value[i][self.layers_names[j]]
                bacc_dict[self.layers_names[j]]      += bacc_value[i][self.layers_names[j]]                                            
                dice_dict[self.layers_names[j]]      += dice_value[i][self.layers_names[j]]                                
                recall_dict[self.layers_names[j]]    += recall_value[i][self.layers_names[j]]                        
                iou_dict[self.layers_names[j]]       += iou_value[i][self.layers_names[j]]
    ...
    #     info = {'Accuracy'           : [res(result_acc_mean[0],result_acc_std[0]), res(result_acc_mean[1],result_acc_std[1])],
    #             'Balanced Accuracy'  : [res(result_bacc_mean[0],result_bacc_std[0]), res(result_bacc_mean[1],result_bacc_std[1])],
    ...
    #             'Dice'               : [res(result_dice_mean[0],result_dice_std[0]), res(result_dice_mean[1],result_dice_std[1])],
    #             'IoU'                : [res(result_iou_mean[0],result_iou_std[0]), res(result_iou_mean[1],result_iou_std[1])]}

    #     print('\n Overall Results on %s'%dataset_name)
    #     print(tabulate(info, headers='keys', tablefmt='fancy_grid',
    #                   showindex=['Background', 'Retina'],
    #                   missingval='N/A'))   
